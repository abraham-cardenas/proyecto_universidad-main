...
const firebaseConfig = {
  ...
  export default class FirebaseOptimizer {
    constructor() {
      if (!initializeApp.apps.length ) {initializeApp.initializeApp(firebaseConfig)}
      console.log('Initializando')
      this.auth = initializeApp.auth();
      this.logged = false;
      this.user = {};
      this.dbUsers = initializeApp.firestore().collection('users'); 
      this.auth.onAuthStateChanged( user => { if (user) {this.user=user; this.logged = true}else{this.user={}; this.logged=false}})
      this.googleProvider = new initializeApp.auth.GoogleAuthProvider();
    }
